# SHK Accounts and User Data – v1 Contract (v1.1, Phase 1)
...
- Confirms `public.profiles` as the canonical app-level record including `role`, `account_status`, `strike_count`, and `show_sensitive` as implemented fields.
- Documents the `/account/search/` account finder page and its header search button.
- Acknowledges basic `/admin/` scaffolding for future moderation and account management while deferring the full moderation system to Phase 2.
...
### 3.5 Account search – `/account/search/`

- Public-facing search surface for finding accounts by handle and/or display name.
- Accessible to guests and signed-in users; returns only public-safe profile fields (no email).
- Results show avatar, display name, and `@handle`, plus a “View profile” link to `/u/?handle=…`.
- Primary entry point is the header search button; additional links may exist from account/admin areas.
- This is intentionally minimal in Phase 1; richer filters and admin-only views are deferred to Phase 2.
...
- A small “Search” pill button with a monochrome magnifying-glass icon links to `/account/search/` and appears to the left of the account pill on larger screens.
...
- This file: **SHK Accounts and User Data – v1 Contract (v1.1, Phase 1)**.
- Stored in repo as: `docs/data/contracts/p1.user_account_data.v1.contract.md` (or equivalent path).
- Changes that affect DB schema, RLS, or public behavior require:
  - Version bump (e.g. v1.2, v1.3).
